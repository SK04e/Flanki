const API_URL = ""; 
    ...
    let autoKickInterval = null; // Globalna zmienna do odliczania wyrzucenia

    window.onload = () => {
        updateAuthUI();
        if(token) {
            switchView('view-profile');
            if(currentActiveLobbyId) fetchLobby();
        }
        ...
        try {
            ...
            if (res.ok) {
                ...
                history.forEach(game => {
                    ...
                    let winColor = (game["zwyciezcy"] && game["zwyciezcy"] === game["Twoja drużyna"]) 
                        ? "color: var(--accent-green); font-weight: bold;" : "color: var(--text-main);";
    ...
    async function fetchActiveGames() {
        ...
        try {
            ...
            // Wdrażamy akordeon (rozwijane lobby) z nazwą Hosta
            games.forEach(game => {
                ...
                listContainer.innerHTML += `
                    <details class="lobby-accordion">
                        ...
                        <div class="lobby-details-content">
                            ...
                            <button class="btn" style="width: auto; padding: 10px 20px; margin-top: 0; font-size: 14px;" onclick="joinGamePrompt(${game.game_id})">Wejdź do Lobby 🔒</button>
                        </div>
                    </details>
                `;
            });
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.game_id) {
                    ...
                    localStorage.setItem('active_lobby_id', data.game_id);
                    switchView('view-lobby');
                    showToast("Przywrócono sesję gry!", "success");
                } else {
    ...
    async function fetchLobby() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Szukamy nazwy hosta po ID
                let hostNameStr = data.host_id;
                if(data.players) {
                    let hostObj = data.players.find(p => p.player_id === data.host_id);
                    if(hostObj) hostNameStr = hostObj.name + ` (ID: ${data.host_id})`;
                }
                        ...
                        listContainer.innerHTML += `
                            <li class="player-item">
                                <div style="display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <span style="font-weight: 600; font-size: 16px;">${player.name}</span> ${isMe}
            ...
            showToast("Błąd łączenia z API", "error"); 
        }
    }
    ...
    async function fetchLeaderboard() {
        ...
        try {
            ...
            players.forEach((p, index) => {
                ...
                const winRatio = p.games_played > 0 
                    ? Math.round((p.games_won / p.games_played) * 100) + '%' 
                    : '0%';

                // Safe strings for inline onclick call
    ...
    const przFaculties = [
        { value: "WEII", label: "WEiI" },
        { value: "WC", label: "WCh" },
        { value: "WZ", label: "WZ" },
        { value: "WMiFS", label: "WMiFS" },
        { value: "WBMiL", label: "WBMiL" },
        { value: "WBIŚiA", label: "WBIŚiA" },
        { value: "WMT", label: "WMT" }
    ];

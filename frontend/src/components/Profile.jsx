...
export default function Profile() {
  ...
  const openMatchDetails = async (gameId) => {
    ...
    try {
      ...
      toast.error("Błąd pobierania szczegółów meczu.");
    } finally {
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-24">
      ...
      <div className="space-y-3">
        <div className="flex justify-between items-center px-1">
          ...
          <button onClick={() => setHideCanceled(!hideCanceled)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${!hideCanceled ? 'bg-yellow-400/10 border-yellow-400/30 text-yellow-400' : 'bg-slate-900 border-slate-800 text-slate-400 hover:text-slate-200'}`}>
        ...
        {loading ? (
          <div className="space-y-2">
            {[1, 2, 3].map((i) => (
              <div key={i} className="h-16 bg-white/[0.02] rounded-xl border border-white/5 animate-pulse flex items-center px-4">
                <div className="w-8 h-8 bg-slate-800/40 rounded-md mr-4" />
                <div className="space-y-2 flex-1"><div className="h-3 w-24 bg-slate-800/40 rounded" /><div className="h-2 w-16 bg-slate-800/30 rounded" /></div>
              </div>
            ))}
          </div>
        ) : filteredHistory.length === 0 ? (
          <div className="text-center py-10 bg-white/[0.02] rounded-2xl border border-dashed border-white/10">
            <p className="text-xs text-slate-400">Brak historii meczów.</p>
          </div>
        ) : (
          <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden backdrop-blur-2xl shadow-2xl">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-white/10 text-[10px] text-slate-400 uppercase tracking-wider bg-white/[0.02]">
                    <th className="py-2.5 px-3 font-bold">ID</th><th className="py-2.5 px-3 font-bold">Data</th><th className="py-2.5 px-3 font-bold">Drużyna</th><th className="py-2.5 px-3 font-bold">Wynik</th><th className="py-2.5 px-3 font-bold">Status</th><th className="py-2.5 px-3 text-right font-bold">Akcja</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5 text-xs">
                  {filteredHistory.slice().reverse().map((match, idx) => {
                    ...
                    const teamName = match['Twoja drużyna'] === '1' || match['Twoja drużyna'] === 'A' ? 'TEAM A' : match['Twoja drużyna'] === '2' || match['Twoja drużyna'] === 'B' ? 'TEAM B' : 'BRAK';
                    
                    let rowHighlight = "hover:bg-white/[0.06]";
                    if (isFinished) rowHighlight = isWin ? "bg-emerald-500/10 hover:bg-emerald-500/20 border-l-4 border-l-emerald-500" : "bg-red-500/10 hover:bg-red-500/20 border-l-4 border-l-red-500";
                    
                    const dateParts = (match['date'] || '').split(' ');
                    return (
                      <tr key={idx} onClick={() => openMatchDetails(match['ID gry'])} className={`transition-colors cursor-pointer group ${rowHighlight}`}>
                        <td className="py-3 px-3 font-black text-yellow-400">#{match['ID gry']}</td>
                        <td className="py-3 px-3 whitespace-nowrap"><div className="flex flex-col"><span className="text-slate-100 font-semibold">{dateParts[0] || match['date']}</span>{dateParts[1] && <span className="text-[10px] text-slate-400 font-medium">{dateParts[1]}</span>}</div></td>
                        <td className="py-3 px-3 font-black tracking-wide text-slate-200">{teamName}</td>
                        <td className="py-3 px-3">{isFinished ? <span className={`px-2 py-0.5 rounded text-[10px] font-black tracking-wider uppercase shadow-sm ${isWin ? 'bg-emerald-500 text-slate-950' : 'bg-red-500 text-white'}`}>{isWin ? 'WIN' : 'LOSS'}</span> : isCanceled ? <span className="text-slate-400 font-bold text-[9px] bg-white/10 px-1.5 py-0.5 rounded">CANCELED</span> : <span className="text-slate-300 font-bold text-[10px]">W TOKU</span>}</td>
                        <td className="py-3 px-3 uppercase text-[9px] font-black tracking-wider text-slate-400">{match['Status gry']}</td>
                        <td className="py-3 px-3 text-right"><button className="text-cyan-400 hover:text-cyan-300 font-bold transition-colors text-[11px] ml-auto">Szczegóły</button></td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      <AnimatePresence>
        {editModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="w-full max-w-sm bg-slate-900/85 backdrop-blur-2xl border border-slate-700/50 rounded-3xl p-6 shadow-[0_0_40px_rgba(0,0,0,0.5)] relative">
              <button onClick={() => setEditModal(false)} className="absolute top-4 right-4 text-slate-400 hover:text-white bg-slate-950/50 border border-slate-800 p-1.5 rounded-full"><X className="w-4 h-4" /></button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl relative max-h-[90vh] flex flex-col">
              <div className="bg-slate-950 p-4 border-b border-slate-800 flex items-center justify-between sticky top-0 z-10">
                <h2 className="text-lg font-black text-white tracking-wider flex items-center gap-2">MATCH ROOM <span className="text-xs font-bold text-yellow-400 bg-yellow-400/10 px-2 py-0.5 rounded">#{selectedMatchId}</span></h2>
                <button onClick={() => setSelectedMatchId(null)} className="text-slate-400 hover:text-white bg-slate-900 p-1.5 rounded-lg transition-colors"><X className="w-5 h-5" /></button>
              </div>
              <div className="p-4 overflow-y-auto">
                {loadingDetails ? <div className="py-20 text-center animate-pulse text-yellow-400 font-bold text-sm tracking-widest uppercase">Pobieranie danych serwera...</div> : matchDetails ? (
                  <div className="space-y-6">
                    <div className="flex justify-around items-center bg-slate-950/50 rounded-xl p-3 border border-slate-800/80">
                      <div className="text-center"><MapPin className="w-4 h-4 text-slate-500 mx-auto mb-1" /><span className="text-[10px] text-slate-400 font-bold uppercase">{matchDetails.location}</span></div>
                      <div className="text-center"><Calendar className="w-4 h-4 text-slate-500 mx-auto mb-1" /><span className="text-[10px] text-slate-400 font-bold uppercase">{matchDetails.end_time ? `${new Date(matchDetails.end_time).toLocaleDateString()} ${new Date(matchDetails.end_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : 'N/A'}</span></div>
                      <div className="text-center"><Trophy className="w-4 h-4 text-slate-500 mx-auto mb-1" /><span className="text-[10px] text-slate-400 font-bold uppercase">{matchDetails.status}</span></div>
                    </div>
                    <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-start">
                      {(() => {
                        const isAWinner = matchDetails.winning_team === 'A' || matchDetails.winning_team === '1';
                        return (
                          <div className={`rounded-2xl p-3 border-t-4 border-x border-b ${isAWinner ? 'border-t-emerald-400 bg-emerald-500/10 border-slate-800 shadow-[0_0_20px_rgba(16,185,129,0.2)]' : 'border-t-slate-600 bg-slate-950/40 border-slate-800/80'}`}>
                            <div className="text-center mb-3"><h3 className="font-black text-sm text-white tracking-wider">TEAM A</h3></div>
                            <div className="space-y-1">{matchDetails.players.filter(p => p.team === 'A' || p.team === '1').map(p => <MatchPlayerCard key={p.player_id} p={p} isWinner={isAWinner} />)}</div>
                          </div>
                        );
                      })()}
                      <div className="font-black text-slate-700 flex flex-col items-center justify-center h-full pt-8">VS</div>
                      {(() => {
                        const isBWinner = matchDetails.winning_team === 'B' || matchDetails.winning_team === '2';
                        return (
                          <div className={`rounded-2xl p-3 border-t-4 border-x border-b ${isBWinner ? 'border-t-emerald-400 bg-emerald-500/10 border-slate-800 shadow-[0_0_20px_rgba(16,185,129,0.2)]' : 'border-t-slate-600 bg-slate-950/40 border-slate-800/80'}`}>
                            <div className="text-center mb-3"><h3 className="font-black text-sm text-white tracking-wider">TEAM B</h3></div>
                            <div className="space-y-1">{matchDetails.players.filter(p => p.team === 'B' || p.team === '2').map(p => <MatchPlayerCard key={p.player_id} p={p} isWinner={isBWinner} />)}</div>
                          </div>
                        );
                      })()}
                    </div>
